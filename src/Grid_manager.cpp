...
void Grid_manager::readFarms(const std::string& farm_fname)
{
	...
	if(!f){std::cout << "Premises file not found. Exiting..." << std::endl; Rcpp::stop("");}
	if(f.is_open())
	{
	    skipBOM(f);
...
// overloaded (alternate) constructor that reads in external file of cells
{
	...
	if(!f){std::cout << "Input file not found. Exiting..." << std::endl; Rcpp::stop("");}
	if(f.is_open())
	{
...
void Grid_manager::read_seedSource(std::string seedSource, std::vector<std::string>& output)
{
	...
	if(!f){std::cout << "Seed source file not found. Exiting..." << std::endl; Rcpp::stop("");}
