	...
	kp(kparams)
{
	...
	switch (kType)
	{
		...
		default:{
			std::cout << "Kernel type must be 0 or 2 if constructing with parameters. Exiting..." << std::endl; Rcpp::stop("");
		}
	}
}
...
std::cout<<"Creating a kernel with datafile "<<datafile<<std::endl;
		// read in levels from file
			...
			if(!d){std::cout << "Data-based local spread file not found. Exiting..." << std::endl; Rcpp::stop("");}
			if(d.is_open()){
...
if(verbose>1){std::cout<<" At "<<distM<<", risk is "<<distProb[distM*distM]<<std::endl;}
		...
		default:{
			std::cout << "Kernel type must be 1 if constructing from file. Exiting..." << std::endl; Rcpp::stop("");
		}
	}
}
...
if(verbose>1){std::cout << "Matched distance "<<std::sqrt(distSq)<<" with probability value of "<<k<<std::endl;}
		...
		default:{
			std::cout << "Unrecognized kernel type. Exiting..." << std::endl; Rcpp::stop("");
		}
	}
